...
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { PodcastSlotsBookingService, Podcast, PodcastBooking, BookingResponse } from '../../../services/podcast.service';
import { swalHelper } from '../../../core/constants/swal-helper';
import { debounceTime, Subject } from 'rxjs';
import { NgxPaginationModule } from 'ngx-pagination';
import { NgSelectModule } from '@ng-select/ng-select';
...
@Component({
  selector: 'app-podcast-bookings',
  standalone: true,
  imports: [CommonModule, FormsModule, NgxPaginationModule, NgSelectModule],
  providers: [PodcastSlotsBookingService],
  // templateUrl: './podcast-booking.component.html',
  // styleUrls: ['./podcast-booking.component.css'],
   templateUrl: './podcastBooking.component.html',
  styleUrls: ['./podcastBooking.component.css'],
})
export class BookingsComponent implements OnInit, AfterViewInit {
  ...
  searchQuery: string = '';
  bookingModal: any;
  selectedBooking: PodcastBooking | null = null;
  ...
  async fetchPodcasts(): Promise<void> {
    this.loadingPodcasts = true;
    try {
      const response = await this.podcastService.getAllPodcasts({
        page: 1,
        limit: 100,
        search: ''
      });
  ...
  openStatusUpdateModal(booking: PodcastBooking): void {
    this.selectedBooking = booking;
    this.bookingUpdate = {
      bookingId: booking._id,
      action: booking.status as any,
  ...
  // Helper method to get booking data for templates
  get bookingDocs(): PodcastBooking[] {
    return this.bookings?.data?.docs || [];
  }

  get totalBookings(): number {
    return this.bookings?.data?.totalDocs || 0;
  }

  get currentPage(): number {
    return this.bookings?.data?.page || 1;
  }

  get totalPages(): number {
    return this.bookings?.data?.totalPages || 1;
  }

  get pagingCounter(): number {
    return this.bookings?.data?.pagingCounter || 1;
  }
}